...
#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct ImportReverseRequest {
    pub batch_id: String,
    pub reason: String,
}
...
#[derive(Debug, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct ImportReverseResult {
    pub batch_id: String,
    pub status: String,
...
async fn validate_scope(
    ...
    let contract_project: Option<String> =
        sqlx::query_scalar("SELECT project_id FROM contracts WHERE id=?")
    ...
    if contract_project.as_deref() != Some(request.project_id.as_str()) {
        return Err(format!(
            "Row {source_row}: selected contract is missing or outside the selected project."
        ));
    }
    let row_project = string_value(row, "project_id");
    if !row_project.is_empty() && row_project != request.project_id {
        return Err(format!(
            "Row {source_row}: project scope does not match the selected project."
        ));
    }
    let row_contract = string_value(row, "contract_id");
    if !row_contract.is_empty() && row_contract != request.contract_id {
        return Err(format!(
            "Row {source_row}: contract scope does not match the selected contract."
        ));
    }
    if request.target_table == "boq_items" {
        let header_id = string_value(row, "boq_header_id");
        let header_contract: Option<String> =
            sqlx::query_scalar("SELECT contract_id FROM boq_headers WHERE id=? AND project_id=?")
                .bind(&header_id)
        ...
        if header_contract.as_deref() != Some(request.contract_id.as_str()) {
            return Err(format!(
                "Row {source_row}: BOQ header is missing or outside the selected contract."
            ));
        }
    } else if request.target_table == "wir_entries"
        || !row.get("boq_item_id").map(Value::is_null).unwrap_or(true)
    {
        ...
        if !item_id.is_empty() {
            ...
            if item_contract.as_deref() != Some(request.contract_id.as_str()) {
                return Err(format!(
                    "Row {source_row}: BOQ item is missing or outside the selected contract."
                ));
            }
        } else if request.target_table == "wir_entries" {
    ...
    if request.target_table == "schedules" {
        ...
        if !calendar_id.is_empty() {
            ...
            let staged = request.auxiliary_rows.iter().any(|entry| {
                entry.table == "work_calendars"
                    && entry
                        .row
                        .as_object()
                        .map(|candidate| string_value(candidate, "id") == calendar_id)
                        .unwrap_or(false)
            });
...
async fn validate_rows(
    ...
    if !supported_table(&request.target_table) {
        return Err("This import target is not governed by atomic import.".into());
    }
    if request.rows.is_empty() && request.updates.is_empty() {
        return Err("The import contains no rows.".into());
    }
    let existing = query_payloads(
        tx,
    ...
    let mut planned_by_item: std::collections::HashMap<String, f64> =
    ...
    for payload in existing {
        ...
        let code_key = match request.target_table.as_str() {
            "boq_items" => "item_code",
            "schedules" => "activity_code",
            _ => "wir_number",
        };
        ...
        if request.target_table == "schedules" {
            *planned_by_item
                .entry(string_value(&object, "boq_item_id"))
                .or_default() += number_value(&object, "planned_quantity");
        }
        if request.target_table == "wir_entries"
            && string_value(&object, "result").to_lowercase() != "fail"
        {
            *wir_by_item
                .entry(string_value(&object, "boq_item_id"))
                .or_default() += number_value(&object, "quantity");
        }
    }
    for (index, value) in request.rows.iter().enumerate() {
        ...
        let row = value
            .as_object()
            .ok_or_else(|| format!("Row {source_row}: import data is not an object."))?;
        if string_value(row, "id").is_empty() {
            return Err(format!(
                "Row {source_row}: missing controlled record identifier."
            ));
        }
        ...
        if code.is_empty() {
            return Err(format!("Row {source_row}: {code_key} is required."));
        }
        if !codes.insert(code) {
            return Err(format!(
                "Row {source_row}: duplicate {code_key}; no rows were saved."
            ));
        }
        if request.target_table == "schedules" || request.target_table == "wir_entries" {
            ...
            if !item_id.is_empty() {
                ...
                let quantity_key = if request.target_table == "schedules" {
                    "planned_quantity"
                } else {
                    "quantity"
                };
                ...
                if amount <= 0.0 {
                    return Err(format!(
                        "Row {source_row}: {quantity_key} must be greater than zero."
                    ));
                }
                let totals = if request.target_table == "schedules" {
                    &mut planned_by_item
                } else {
                    &mut wir_by_item
                };
                ...
                *total += amount;
                if *total > capacity.unwrap_or(0.0) + 0.000001 {
                    return Err(format!("Row {source_row}: cumulative {quantity_key} exceeds the BOQ quantity; no rows were saved."));
                }
            }
        }
    }
...
async fn validate_updates(
    ...
    if request.target_table != "schedules" {
        return Err(
            "Only schedule planning refreshes may update existing imported records.".into(),
        );
    }
    ...
    for (index, update) in request.updates.iter().enumerate() {
        if !matches!(
            update.table.as_str(),
            "schedules" | "schedule_resource_assignments"
        ) || update.id.trim().is_empty()
        {
            return Err(format!("Planning refresh row {} is invalid.", index + 1));
        }
        if !ids.insert(format!("{}:{}", update.table, update.id.to_lowercase())) {
            return Err(format!(
                "Planning refresh row {} repeats a record.",
                index + 1
            ));
        }
        ...
        let patch = update
            .patch
            .as_object()
            .ok_or_else(|| format!("Planning refresh row {} has an invalid patch.", index + 1))?;
        if patch.keys().any(|key| !allowed.contains(key.as_str())) {
            return Err(format!("Planning refresh row {} attempts to change protected progress, quantity, cost, code, or scope data.", index + 1));
        }
        let before_text: Option<String> = sqlx::query_scalar(&format!(
            "SELECT payload FROM {} WHERE id=? AND project_id=? AND contract_id=?",
            update.table
        ))
        .bind(&update.id)
        .bind(&request.project_id)
        ...
        .await
        .map_err(|error| error.to_string())?;
        let before_text = before_text.ok_or_else(|| {
            format!(
                "Planning refresh row {} is missing or outside the selected project and contract.",
                index + 1
            )
        })?;
        let before: Value =
            serde_json::from_str(&before_text).map_err(|error| error.to_string())?;
        let merged = {
...
async fn validate_auxiliary_rows(
    tx: &mut Transaction<'_, Sqlite>,
    request: &ImportCommitRequest,
) -> Result<(), String> {
    let mut wbs_codes = HashSet::new();
    let mut calendar_codes = HashSet::new();
    let staged_resource_types: HashMap<String, String> = request
        .auxiliary_rows
        .iter()
        .filter(|auxiliary| auxiliary.table == "resource_masters")
        .filter_map(|auxiliary| {
            auxiliary
                .row
                .as_object()
                .map(|row| (string_value(row, "id"), string_value(row, "resource_type")))
        })
        .collect();
    let staged_schedule_ids: HashSet<String> = request
        .rows
        .iter()
    ...
    for (index, auxiliary) in request.auxiliary_rows.iter().enumerate() {
        if !matches!(
            auxiliary.table.as_str(),
            "wbs_nodes" | "work_calendars" | "resource_masters" | "schedule_resource_assignments"
        ) {
            return Err("Unsupported supporting import table.".into());
        }
        let label = match auxiliary.table.as_str() {
            "wbs_nodes" => "WBS",
            "work_calendars" => "Work calendar",
            "resource_masters" => "Resource master",
            _ => "Resource assignment",
        };
        let row = auxiliary
            .row
            .as_object()
            .ok_or_else(|| format!("{label} row {}: import data is not an object.", index + 1))?;
        let code_field = match auxiliary.table.as_str() {
            "wbs_nodes" => "wbs_code",
            "work_calendars" => "calendar_code",
            "resource_masters" => "resource_code",
            _ => "id",
        };
        if string_value(row, "id").is_empty() || string_value(row, code_field).is_empty() {
            return Err(format!(
                "{label} row {}: a controlled identifier and {code_field} are required.",
                index + 1
            ));
        }
        if string_value(row, "project_id") != request.project_id {
            return Err(format!(
                "{label} row {}: project scope does not match the selected project.",
                index + 1
            ));
        }
        ...
        if !contract_id.is_empty() && contract_id != request.contract_id {
            return Err(format!(
                "{label} row {}: contract scope does not match the selected contract.",
                index + 1
            ));
        }
        let code = string_value(row, code_field).to_lowercase();
        if auxiliary.table == "wbs_nodes" {
            if !wbs_codes.insert(code.clone()) {
                return Err(format!(
                    "WBS row {}: duplicate WBS code in this import batch.",
                    index + 1
                ));
            }
            ...
            if exists.is_some() {
                return Err(format!(
                    "WBS row {}: WBS code already exists in the selected project.",
                    index + 1
                ));
            }
            ...
            if !matches!(
                pattern.as_str(),
                "Calendar Days" | "5-Day Week" | "6-Day Week" | "24/7" | "Custom"
            ) {
                return Err(format!(
                    "Work calendar row {}: working pattern is not governed.",
                    index + 1
                ));
            }
            if pattern == "Custom" && string_value(row, "calendar_working_days").is_empty() {
                return Err(format!(
                    "Work calendar row {}: custom pattern requires explicit working days.",
                    index + 1
                ));
            }
            let hours_per_day = row
                .get("hours_per_day")
                .and_then(Value::as_f64)
                .unwrap_or(8.0);
            if !(hours_per_day > 0.0 && hours_per_day <= 24.0) {
                return Err(format!(
                    "Work calendar row {}: hours per working day must be between 0 and 24.",
                    index + 1
                ));
            }
            if !calendar_codes.insert(code.clone()) {
                return Err(format!(
                    "Work calendar row {}: duplicate calendar code in this import batch.",
                    index + 1
                ));
            }
            ...
            if exists.is_some() {
                return Err(format!(
                    "Work calendar row {}: calendar code already exists.",
                    index + 1
                ));
            }
        } else if auxiliary.table == "resource_masters" {
            let kind = string_value(row, "resource_type");
            if !matches!(kind.as_str(), "Labor" | "Equipment")
                || string_value(row, "resource_name").is_empty()
            {
                return Err(format!(
                    "Resource master row {}: name and governed Labor/Equipment type are required.",
                    index + 1
                ));
            }
            ...
            if exists.is_some() {
                return Err(format!(
                    "Resource master row {}: resource code already exists.",
                    index + 1
                ));
            }
        } else {
            if string_value(row, "schedule_id").is_empty()
                || string_value(row, "resource_id").is_empty()
                || !matches!(
                    string_value(row, "resource_type").as_str(),
                    "Labor" | "Equipment"
                )
            {
                return Err(format!("Resource assignment row {}: activity, resource and governed type are required.", index + 1));
            }
            ...
            let schedule_exists = staged_schedule_ids.contains(&schedule_id)
                || sqlx::query_scalar::<_, String>(
                    "SELECT id FROM schedules WHERE id=? AND project_id=? AND contract_id=?",
                )
                .bind(&schedule_id)
                .bind(&request.project_id)
                .bind(&request.contract_id)
                ...
                .is_some();
            if !schedule_exists {
                return Err(format!("Resource assignment row {}: activity does not exist in the selected project/contract or this import batch.", index + 1));
            }
            let master_type = if let Some(kind) = staged_resource_types.get(&resource_id) {
                Some(kind.clone())
            } else {
            ...
            if master_type.as_deref() != Some(resource_type.as_str()) {
                return Err(format!("Resource assignment row {}: selected resource is missing or its type does not match the assignment.", index + 1));
            }
        }
    }
    Ok(())
...
async fn insert_target(
    ...
    let created = string_value(row, "created_at");
...
async fn update_payload(
    ...
    if !matches!(
        table,
        "boq_items" | "contracts" | "schedules" | "schedule_resource_assignments"
    ) {
        return Err("Only BOQ item, contract, schedule, and resource-planning updates are allowed in an import batch.".into());
    }
    let before_text: String =
        sqlx::query_scalar(&format!("SELECT payload FROM {table} WHERE id=?"))
            .bind(id)
            .fetch_optional(&mut **tx)
            .await
    ...
    let after_object = after
        .as_object_mut()
        .ok_or("Derived update payload is not an object.")?;
    for (key, value) in patch
        .as_object()
        .ok_or("Derived update patch is not an object.")?
    {
        after_object.insert(key.clone(), value.clone());
    }
    if table == "boq_items" {
        sqlx::query("UPDATE boq_items SET project_id=?,boq_header_id=?,payload=? WHERE id=?")
            .bind(string_value(after_object, "project_id"))
            .bind(string_value(after_object, "boq_header_id"))
        ...
        sqlx::query(
            "UPDATE contracts SET project_id=?,parent_main_contract_id=?,payload=? WHERE id=?",
        )
        .bind(string_value(after_object, "project_id"))
        .bind(string_value(after_object, "parent_main_contract_id"))
        .bind(after.to_string())
        .bind(id)
        .execute(&mut **tx)
        .await
    ...
    Ok((
        serde_json::from_str(&before_text).map_err(|e| e.to_string())?,
        after,
    ))
}

pub async fn commit_governed_import(
    database_path: &Path,
    request: ImportCommitRequest,
    ...
    match outcome {
        Ok(committed_at) => {
            ...
            Ok(ImportCommitResult {
                ...
                committed_count: request.rows.len() + request.updates.len(),
                committed_at,
            })
        }
        Err(error) => {
...
pub async fn reverse_governed_import(
    database_path: &Path,
    request: ImportReverseRequest,
) -> Result<ImportReverseResult, String> {
    ...
    let status: Option<String> = sqlx::query_scalar("SELECT status FROM import_batches WHERE id=?")
        .bind(&request.batch_id)
        .fetch_optional(&mut *tx)
        .await
        .map_err(|e| e.to_string())?;
    if status.as_deref() != Some("Committed") {
        return Err("Only a committed import batch can be reversed.".into());
    }
    ...
    for (table, id, row_status, source) in &rows {
        if row_status == "Committed" {
            sqlx::query(&format!("DELETE FROM {table} WHERE id=?")).bind(id).execute(&mut *tx).await.map_err(|e| format!("Cannot reverse batch because {table} record {id} is referenced by later data: {e}"))?;
        }
    ...
    sqlx::query("UPDATE import_batches SET status='Reversed', summary_json=? WHERE id=?")
        .bind(json!({"reversal_reason":request.reason,"reversed_at":now()}).to_string())
        .bind(&request.batch_id)
        .execute(&mut *tx)
        .await
        .map_err(|e| e.to_string())?;
    tx.commit().await.map_err(|e| e.to_string())?;
    Ok(ImportReverseResult {
        batch_id: request.batch_id,
        status: "Reversed".into(),
        reversed_count: rows.len(),
    })
}
...
mod tests {
    ...
    async fn setup(path: &Path) {
        ...
        sqlx::query("INSERT INTO projects VALUES ('p1')")
            ...
            .await
            .unwrap();
        sqlx::query("INSERT INTO contracts VALUES ('c1','p1')")
            .execute(&pool)
            .await
            .unwrap();
        sqlx::query("INSERT INTO boq_headers VALUES ('h1','p1','c1')")
            .execute(&pool)
            .await
            .unwrap();
        pool.close().await;
    ...
    async fn late_duplicate_rolls_back_every_target_and_audit_row() {
        let path =
            std::env::temp_dir().join(format!("buildtrack-import-{}.db", std::process::id()));
        let _ = std::fs::remove_file(&path);
        setup(&path).await;
        let request = ImportCommitRequest {
            batch_id: "batch-1".into(),
            source: "Excel".into(),
            file_name: "boq.xlsx".into(),
            target_table: "boq_items".into(),
            project_id: "p1".into(),
            contract_id: "c1".into(),
            rows: vec![
        ...
        .unwrap();
        assert_eq!(
            sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM boq_items")
                .fetch_one(&pool)
                .await
                .unwrap(),
            0
    ...
    async fn reverse_removes_the_entire_committed_batch() {
        let path = std::env::temp_dir().join(format!(
            "buildtrack-import-reverse-{}.db",
            std::process::id()
        ));
        let _ = std::fs::remove_file(&path);
        setup(&path).await;
        let request = ImportCommitRequest {
            batch_id: "batch-reverse".into(),
            source: "Excel".into(),
            file_name: "boq.xlsx".into(),
            target_table: "boq_items".into(),
            project_id: "p1".into(),
        ...
        let reversed = reverse_governed_import(
            &path,
            ImportReverseRequest {
                batch_id: "batch-reverse".into(),
                reason: "acceptance test".into(),
            },
        )
        .await
        .unwrap();
        ...
        assert_eq!(
            sqlx::query_scalar::<_, String>(
                "SELECT status FROM import_batches WHERE id='batch-reverse'"
            )
            .fetch_one(&pool)
            .await
            .unwrap(),
    ...
    async fn schedule_import_commits_and_reverses_its_supporting_masters() {
        let path =
            std::env::temp_dir().join(format!("buildtrack-import-wbs-{}.db", std::process::id()));
        let _ = std::fs::remove_file(&path);
        setup(&path).await;
        let request = ImportCommitRequest {
            batch_id: "batch-wbs".into(),
            source: "Primavera".into(),
            file_name: "schedule.xer".into(),
            target_table: "schedules".into(),
            project_id: "p1".into(),
            contract_id: "c1".into(),
            rows: vec![
                json!({"id":"a1","created_at":"2026-01-01T00:00:00Z","project_id":"p1","contract_id":"c1","activity_code":"ACT-01","activity":"Excavate","wbs_id":"w1","calendar_id":"cal-6d"}),
            ],
            updates: vec![],
            derived_patches: vec![],
            auxiliary_rows: vec![
                ImportAuxiliaryRow {
                    table: "wbs_nodes".into(),
                    row: json!({"id":"w1","created_at":"2026-01-01T00:00:00Z","project_id":"p1","contract_id":"c1","wbs_code":"BLD.10","name":"Structure"}),
                },
                ImportAuxiliaryRow {
                    table: "work_calendars".into(),
                    row: json!({"id":"cal-6d","created_at":"2026-01-01T00:00:00Z","project_id":"p1","contract_id":"c1","calendar_code":"P6-P1-SIX-DAY","calendar_name":"Six Day Calendar","working_pattern":"6-Day Week","status":"Active"}),
                },
            ],
        };
        ...
        assert_eq!(
            sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM wbs_nodes")
                .fetch_one(&pool)
                .await
                .unwrap(),
            1
        ...
        assert_eq!(
            sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM work_calendars")
                .fetch_one(&pool)
                .await
                .unwrap(),
            1
    ...
    async fn planning_refresh_updates_dates_but_preserves_actuals_and_reverses() {
        ...
        let request = ImportCommitRequest {
            ...
            updates: vec![ImportUpdate {
                table: "schedules".into(),
                id: "a1".into(),
                patch: json!({"start_date":"2026-01-03","end_date":"2026-01-08","duration_days":6,"activity":"Excavate revised"}),
            }],
            derived_patches: vec![],
            auxiliary_rows: vec![],
        };
        assert_eq!(
            commit_governed_import(&path, request)
                .await
                .unwrap()
                .committed_count,
            1
        ...
        let refreshed: Value = serde_json::from_str(
            &sqlx::query_scalar::<_, String>("SELECT payload FROM schedules WHERE id='a1'")
                .fetch_one(&pool)
                .await
                .unwrap(),
        )
    ...
    async fn resource_assignment_refresh_updates_once_and_reverses_without_duplication() {
        let path = std::env::temp_dir().join(format!(
            "buildtrack-import-resource-refresh-{}.db",
            std::process::id()
        ));
        let _ = std::fs::remove_file(&path);
        setup(&path).await;
        ...
        pool.close().await;
        let request = ImportCommitRequest {
            batch_id: "batch-resource-refresh".into(),
            source: "Primavera".into(),
            file_name: "schedule.xer".into(),
            target_table: "schedules".into(),
            project_id: "p1".into(),
            ...
            rows: vec![],
            updates: vec![ImportUpdate {
                table: "schedule_resource_assignments".into(),
                id: "ra1".into(),
                patch: json!({"resource_type":"Labor","assignment_start":"2026-01-03","assignment_end":"2026-01-08","planned_hours":56,"planned_quantity":0,"planned_cost":1120,"notes":"P6 refresh"}),
            }],
            derived_patches: vec![],
            auxiliary_rows: vec![],
        };
        ...
        .unwrap();
        assert_eq!(
            sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM schedule_resource_assignments")
                .fetch_one(&pool)
                .await
                .unwrap(),
            1
        ...
        let refreshed: Value = serde_json::from_str(
            &sqlx::query_scalar::<_, String>(
                "SELECT payload FROM schedule_resource_assignments WHERE id='ra1'",
            )
            .fetch_one(&pool)
            .await
            .unwrap(),
