...
pub async fn reverse_claim_conversion(
    ...
    let previous = match previous_operation(&mut tx, &req.operation_id, &req.claim_id).await {
        ...
        Err(error) => {
            return match tx.rollback().await {
                Ok(()) => Err(error),
                Err(rollback_error) => Err(format!(
                    "{}; failed to roll back claims reversal transaction: {}",
                    error, rollback_error
                )),
            };
        }
    ...
    let result: Result<ClaimOperationResult, String> = async {
        ...
        if json_text(&variation, "status") != "Draft" {
            return Err(
                "Only the unapproved Draft variation generated by this claim can be reversed."
                    .into(),
            );
        }
...
mod tests {
    ...
    async fn reversal_respects_period_lock_and_preserves_variation_history() {
        ...
        .is_err());
        let pool = open_pool(&path).await.unwrap();
        let guards: i64 = sqlx::query_scalar("SELECT count(*) FROM claims_mutation_guard")
            .fetch_one(&pool)
            .await
            .unwrap();
        assert_eq!(
            guards, 0,
            "A rejected reversal must not leave a mutation guard."
        );
        pool.close().await;
