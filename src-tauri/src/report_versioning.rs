...
pub async fn issue_report_version(
    path: &Path,
    request: IssueReportVersionRequest,
) -> Result<IssueReportVersionResult, String> {
    for (value, label) in [
    ...
    if !iso_day(&request.data_date) {
        return Err("Report Data Date must use YYYY-MM-DD.".into());
    }
...
mod tests {
    ...
    fn request(id: &str, code: &str) -> IssueReportVersionRequest {
        IssueReportVersionRequest {
            ...
            data_date: "2026-09-06".into(),
            pack_type: "Monthly PMO Review".into(),
            template_id: None,
            version_code: code.into(),
            snapshot_hash: format!("sha256:{}", "a".repeat(64)),
            snapshot_payload: "{\"metrics\":{\"ev\":100}}".into(),
            issuer: "PMO Admin".into(),
            sign_off_note: "Reviewed and issued".into(),
            issued_at: "2026-09-06T10:00:00Z".into(),
    ...
    async fn issuance_supersedes_prior_pack_atomically_and_audits_each_issue() {
        ...
        let first = issue_report_version(&path, request("r1", "RPT-001"))
            .await
            .unwrap();
        assert!(first.superseded_ids.is_empty());
        let second = issue_report_version(&path, request("r2", "RPT-002"))
            .await
            .unwrap();
        assert_eq!(second.superseded_ids, vec!["r1"]);
        let pool = database(&path).await.unwrap();
        let rows: Vec<(String, String, Option<String>)> =
            sqlx::query_as("SELECT id,status,superseded_by FROM report_versions ORDER BY id")
                .fetch_all(&pool)
                .await
                .unwrap();
        assert_eq!(
            rows,
            vec![
                ("r1".into(), "Superseded".into(), Some("r2".into())),
                ("r2".into(), "Issued".into(), None)
            ]
        );
    ...
    async fn issuance_rejects_non_sha256_snapshot_before_writing() {
        ...
        assert!(issue_report_version(&path, invalid)
            .await
            .unwrap_err()
            .contains("SHA-256"));
        let pool = database(&path).await.unwrap();
        let count: i64 = sqlx::query_scalar("SELECT count(*) FROM report_versions")
            .fetch_one(&pool)
            .await
            .unwrap();
        assert_eq!(count, 0);
    ...
    async fn template_approval_is_atomic_audited_and_cannot_be_repeated() {
        ...
        let approved = approve_report_template(
            &path,
            ApproveReportTemplateRequest {
                id: "tpl-1".into(),
                project_id: Some("p1".into()),
                approver: "PMO Admin".into(),
            },
        )
        .await
        .unwrap();
        ...
        let repeated = approve_report_template(
            &path,
            ApproveReportTemplateRequest {
                id: "tpl-1".into(),
                project_id: Some("p1".into()),
        ...
        let status: String = sqlx::query_scalar(
            "SELECT json_extract(payload,'$.status') FROM report_templates WHERE id='tpl-1'",
        )
        .fetch_one(&pool)
        .await
        .unwrap();
        let approved_at: String = sqlx::query_scalar(
            "SELECT json_extract(payload,'$.approved_at') FROM report_templates WHERE id='tpl-1'",
        )
        .fetch_one(&pool)
        .await
        .unwrap();
...
pub async fn approve_report_template(
    path: &Path,
    request: ApproveReportTemplateRequest,
) -> Result<ApproveReportTemplateResult, String> {
    ...
    let outcome: Result<(), String> = async {
        let current_payload: Option<String> =
            sqlx::query_scalar("SELECT payload FROM report_templates WHERE id = ?")
                .bind(&request.id)
    ...
    match outcome {
        Ok(_) => {
            tx.commit().await.map_err(|error| error.to_string())?;
            Ok(ApproveReportTemplateResult {
                id: request.id,
