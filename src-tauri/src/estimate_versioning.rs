...
pub struct ApproveEstimateRequest {
    pub version: Value,
}

#[derive(Debug, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct ApproveEstimateResult {
    pub id: String,
    pub status: String,
...
pub async fn approve_estimate(
...
) -> Result<ApproveEstimateResult, String> {
    ...
    let code = text(&version, "version_code").to_string();
    ...
    if text(&version, "status") != "Approved" {
        return Err("This workflow only approves estimate versions.".into());
    }
    if !iso_day(&data_date) {
        return Err("Estimate Data Date must use YYYY-MM-DD.".into());
    }
    if text(&version, "owner").is_empty()
        || text(&version, "reason").is_empty()
        || text(&version, "assumptions").is_empty()
    {
    ...
    let lines = version
        ...
        .ok_or("Approved estimate requires a forecast line.")?;
    if lines.len() != 1 || text(&lines[0], "control_account_id") != account_id {
        return Err(
            "Estimate version must contain exactly one line for its selected Control Account."
                .into(),
        );
    }
    let line = &lines[0];
    let fac = number(line, "fac");
    let floor = number(line, "actual_cost") + number(line, "open_commitment");
    let waived = line
        .get("waiver_documented")
        .and_then(Value::as_bool)
        .unwrap_or(false)
        && !text(line, "waiver_reason").is_empty();
    if fac + 0.01 < floor && !waived {
        return Err(
            "FAC cannot be below AC plus open commitment without a documented waiver.".into(),
        );
    }
    ...
    match outcome {
        ...
        Err(e) => {
            tx.rollback().await.map_err(|r| r.to_string())?;
            Err(e)
        }
    }
}
...
mod tests {
    ...
    fn version(id: &str, code: &str, line_id: &str) -> Value {
        json!({
    ...
    async fn approval_is_atomic_when_a_new_line_fails() {
        ...
        approve_estimate(
            ...
            ApproveEstimateRequest {
                version: version("e1", "EST-001", "shared"),
            },
        )
        .await
        .unwrap();
        assert!(approve_estimate(
            &path,
            ApproveEstimateRequest {
                version: version("e2", "EST-002", "shared")
            }
        )
        .await
        .is_err());
        let pool = database(&path).await.unwrap();
        let status: String =
            sqlx::query_scalar("SELECT status FROM estimate_versions WHERE id='e1'")
                .fetch_one(&pool)
                .await
                .unwrap();
        let count: i64 = sqlx::query_scalar("SELECT count(*) FROM estimate_versions WHERE id='e2'")
            .fetch_one(&pool)
            .await
            .unwrap();
    ...
    async fn fac_floor_is_rejected_before_any_write() {
        let path = fixture().await;
        let mut invalid = version("e1", "EST-001", "l1");
        invalid["lines"][0]["fac"] = json!(40.0);
        assert!(
            approve_estimate(&path, ApproveEstimateRequest { version: invalid })
                .await
                .unwrap_err()
                .contains("FAC cannot")
        );
        let pool = database(&path).await.unwrap();
        let count: i64 = sqlx::query_scalar("SELECT count(*) FROM estimate_versions")
            .fetch_one(&pool)
            .await
            .unwrap();
        assert_eq!(count, 0);
