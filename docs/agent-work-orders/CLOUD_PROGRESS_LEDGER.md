...
- راجع Codex المستودع الفرعي حتى `941e4c9` وحافظ على الأجزاء المفيدة من F3–G3.
- حُذفت أدوات `fix_*.cjs` و`patch_*.cjs` المؤقتة و`bun.lock` وبيانات السحابة.
- صُحح فشل اعتماد قالب التقرير دون إضافة `chrono` أو `uuid`.
- صُححت Fixtures اختبارات Rust كي تختبر كتابة سجل التدقيق فعليًا.
- أُعيد مسار إنشاء أول مدير محلي الآمن، وأزيل login الوهمي `admin/admin` والتعارض
  مع مخطط `app_users` القائم.
