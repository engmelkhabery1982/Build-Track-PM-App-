...
1. استخدم مستودع `engmelkhabery1982/BuildTrack-Agent-Cloud` والفرع المحدد في
   `CLOUD_BASE_BRANCH` داخل `docs/agent-work-orders/ACTIVE.md`. لا تنشئ repo جديدًا.
...
10. سلم `<Wxx>_RESULT.md` وEvidence JSON. لا تكتب CLOSED أو 8/10 ولا تعدل المؤشر.
    النتيجة فقط `READY FOR CODEX REVIEW` أو `WIP/BLOCKED`، ثم توقف لمراجعة Codex.
