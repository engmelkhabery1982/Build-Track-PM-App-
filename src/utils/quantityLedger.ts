...
export function buildQuantityLedger(input: {
  ...
  return input.boqItems.filter((item) => !item.main_boq_item_id).map((item) => {
    ...
    const acceptedBeforeCorrections = wirs.filter((row) => ['Pass', 'Conditional Pass'].includes(String(row.result || '')) || row.status === 'Approved')
      .reduce((sum, row) => sum + (Number(row.quantity) || 0), 0);
    const correctedQuantity = (input.progressCorrections || [])
      .filter((row) => row.status === 'Posted' && mainIdFor(row.boq_item_id) === itemId)
      .reduce((sum, row) => sum + (String(row.correction_type) === 'Reinstatement' ? 1 : -1) * (Number(row.quantity) || 0), 0);
    const accepted = Math.max(0, acceptedBeforeCorrections + correctedQuantity);
    ...
    return {
      ...
      planned_quantity: qty(planned), inspected_quantity: qty(inspected), corrected_quantity: qty(correctedQuantity), accepted_quantity: qty(accepted),
