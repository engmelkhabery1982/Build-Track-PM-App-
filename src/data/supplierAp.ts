...
export const amendPurchaseOrder = (request: { operationId: string; procurementId: string; actor: string; amendedAt: string; reason: string; quantity: number; unitCost: number; totalCost: number; deliveryDate: string }) => invokeAp<SupplierApOperationResult>('amend_purchase_order', request);
